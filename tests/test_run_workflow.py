...
def test_run_workflow_passes_llm_only_priority_mode_to_module_3(monkeypatch, tmp_path):
    extracted_demands_path = tmp_path / "fixed_demands.json"
    output_dir = tmp_path / "results"
    extracted_demands_path.write_text(
        json.dumps(
    ...
    monkeypatch.setattr(workflow_module, "create_openai_client", lambda *_args, **_kwargs: object())
    monkeypatch.setattr(
        workflow_module,
        "extract_demands_offline",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError("Module 2 should be skipped")),
    ...
    def fake_adjust_weights(demands, client, model, city_context=None, temperature=0.0, mode="hybrid"):
        captured["demand_ids"] = [d["demand_id"] for d in demands]
        captured["mode"] = mode
        captured["model"] = model
        return {
            "global_weights": {"w_distance": 1.0, "w_time": 1.0, "w_risk": 1.0},
            "demand_configs": [{"demand_id": demands[0]["demand_id"], "priority": 2, "reasoning": "llm"}],
            "supplementary_constraints": [],
        }

    monkeypatch.setattr(workflow_module, "adjust_weights", fake_adjust_weights)
    monkeypatch.setattr(workflow_module, "serialize_workflow_results", lambda results: results)
    ...
    assert captured["demand_ids"] == ["REQ000"]
    assert captured["mode"] == "llm-only"
    assert captured["model"] == "demo-model"
    run_meta = json.loads((output_dir / "run_for_test" / "run_meta.json").read_text(encoding="utf-8"))
    assert run_meta["priority_mode"] == "llm-only"
