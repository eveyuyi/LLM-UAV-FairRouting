...
PRIORITY_MODE="${PRIORITY_MODE:-llm-only}"

# Reserved default port segment for llm-only 3GPU runs.
...
mkdir -p logs
FAILED_LOG="logs/${PRIORITY_MODE}_failed_cases.log"
touch "${FAILED_LOG}"
...
eval_one() {
  ...
  if ! CONDA_ENV="${CONDA_ENV}" OPENAI_API_KEY="${OPENAI_API_KEY}" \
    ...
    PRIORITY_MODE="${PRIORITY_MODE}" RANK_ONLY_MODE=llm3_only AUTO_TIMESTAMP_OUTPUT_ROOT=0 \
    OUTPUT_ROOT="${out}" \
